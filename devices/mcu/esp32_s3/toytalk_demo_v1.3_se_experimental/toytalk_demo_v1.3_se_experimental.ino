...
// ==== SE再生タスク ====
TaskHandle_t seTaskHandle = NULL;
volatile bool seTaskRunning = false;

// ==== SE用バッファ（起動時に確保、再利用） ====
int16_t* seStereoBuf = NULL;
size_t seStereoLen = 0;
...
// ==== SE初期化（起動時に一度だけ呼ぶ） ====
void initSE() {
  Serial.println("[SE] Initializing...");

  // ステレオ変換済みバッファを確保
  size_t samples = SE_INPUT_END_LEN / 2;
  seStereoLen = samples * 4;

  seStereoBuf = (int16_t*)ps_malloc(seStereoLen);
  if (!seStereoBuf) {
    Serial.println("[SE] ps_malloc failed");
    return;
  }

  // PROGMEMから読み込んでステレオ変換
  uint8_t* tempMono = (uint8_t*)ps_malloc(SE_INPUT_END_LEN);
  if (!tempMono) {
    Serial.println("[SE] temp malloc failed");
    free(seStereoBuf);
    seStereoBuf = NULL;
    return;
  }

  memcpy_P(tempMono, SE_INPUT_END, SE_INPUT_END_LEN);
  monoToStereo((int16_t*)tempMono, seStereoBuf, samples);
  free(tempMono);

  Serial.printf("[SE] Ready: %d bytes stereo\n", seStereoLen);
}

// ==== 効果音再生タスク ====
void sePlayTask(void* param) {
  seTaskRunning = true;
  Serial.println("🔔 [SE Task] Starting...");

  if (!seStereoBuf || seStereoLen == 0) {
    Serial.println("[SE Task] Buffer not initialized");
    seTaskRunning = false;
    seTaskHandle = NULL;
    vTaskDelete(NULL);
    return;
  ...
  size_t offset = 0;
  while (offset < seStereoLen && seTaskRunning) {
    size_t toWrite = min(CHUNK_SIZE, seStereoLen - offset);
    size_t written = 0;
    i2s_write(I2S_NUM_1, (uint8_t*)seStereoBuf + offset, toWrite, &written, pdMS_TO_TICKS(100));
    offset += written;
  }

  Serial.println("[SE Task] Done");
  seTaskRunning = false;
  seTaskHandle = NULL;
  vTaskDelete(NULL);
}
...
void playSEInputEnd() {
  // 前のタスクがまだ動いていたら停止
  if (seTaskHandle != NULL) {
    seTaskRunning = false;
    vTaskDelay(pdMS_TO_TICKS(50));
    if (seTaskHandle != NULL) {
      vTaskDelete(seTaskHandle);
      seTaskHandle = NULL;
    }
  }

  // 新しいタスクを起動（Core 0で実行してメイン処理と分離）
...
void stopSETask() {
  if (seTaskHandle != NULL) {
    seTaskRunning = false;
    vTaskDelay(pdMS_TO_TICKS(100));  // 終了を待つ
    if (seTaskHandle != NULL) {
      vTaskDelete(seTaskHandle);
      seTaskHandle = NULL;
    }
    Serial.println("[SE] Task stopped for TTS");
  }
}
...
void setup() {
  Serial.begin(921600);
  delay(100);
  Serial.println("\n🚀 ToyTalk Conversation v1.3_se_experimental (STT→LLM→TTS with Streaming Chunk Playback)");
