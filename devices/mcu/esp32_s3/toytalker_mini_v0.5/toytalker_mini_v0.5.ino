// toytalker_mini_v0.5【実験版・展示会には使わない】
// v0.4ベース + マイク送信ジッタバッファ（細い回線での録音サンプル欠落→化け認識対策）
// 録音(micCaptureTask)と送信(loop)を分離し、PSRAMリングバッファで上り回線の詰まりを吸収
//
// v0.4での実験時の既知の不具合: 発話すると文字起こしが止まる。本版での対策仮説:
//  A) sendBIN失敗→リトライ時の二重マスク問題（WebSocketsClientはペイロードを
//     in-placeでXORマスクすることがある）→ 内部RAMのステージングバッファ経由で送信し、
//     リング本体を書き換えさせない＆リトライは常に元データから
//  B) 排出ループ中にws.loop()を呼んでおらず受信(partial)処理が遅延 → 送信の合間に呼ぶ
//
// 安定版はv0.4（このマイクバッファなし）。ダメならv0.4に書き込み直せば即復旧
...
// ==== マイク送信ジッタバッファ（PSRAMリングバッファ + 録音専用タスク） ====
// 上り回線が瞬間的に詰まって ws.sendBIN がブロックしても、録音タスクは止まらず
// リングバッファに溜め続けるため、録音サンプルの欠落（虫食い音声→化け認識）を防ぐ。
// 再生側（playRing）と対称の構造。
#define MIC_RING_SIZE    (256 * 1024)  // 256KB ≈ 8秒分 (16kHz/16bit/mono)
#define MIC_READ_SAMPLES 512           // 1回のi2s_read量（32ms分）
#define MIC_SEND_CHUNK   2048          // 1回のws.sendBINの最大量

static uint8_t* micRing = NULL;
static volatile size_t micRingHead = 0;     // producer(録音タスク)書き込み位置
static volatile size_t micRingTail = 0;     // consumer(loop送信側)読み出し位置
static volatile bool micTaskActive = false; // 録音セッション中
static volatile bool micTaskParked = true;  // タスクがI2Sに触っていない状態
static volatile bool micFlushPending = false; // 送信開始時にWS接続前の古い音声を捨てるフラグ
static TaskHandle_t micTaskHandle = NULL;

size_t micRingAvail() {
  size_t h = micRingHead, t = micRingTail;
  return (h + MIC_RING_SIZE - t) % MIC_RING_SIZE;
}

size_t micRingFree() {
  return MIC_RING_SIZE - 1 - micRingAvail();
}

// producer: 録音専用タスク（Core 0）。I2S→16bit変換→リングへ書くだけ
void micCaptureTask(void* param) {
  ...
  for (;;) {
    ...
    size_t n = 0;
    i2s_read(I2S_NUM_0, (void*)raw, sizeof(raw), &n, pdMS_TO_TICKS(100));
    ...
    if (micRingFree() < bytes) continue;  // 8秒分詰まったら以降は破棄（実質圏外状態）

    size_t h = micRingHead;
    size_t n1 = min(bytes, MIC_RING_SIZE - h);  // 折返し前の連続領域
    memcpy(micRing + h, (uint8_t*)pcm, n1);
    if (bytes > n1) memcpy(micRing, (uint8_t*)pcm + n1, bytes - n1);
    micRingHead = (h + bytes) % MIC_RING_SIZE;
  }
}

// 録音セッション開始（I2S_NUM_0インストール後に呼ぶ）
void micCaptureStart() {
  if (!micRing) return;
  micRingHead = 0;
  micRingTail = 0;
  micFlushPending = true;  // WS接続完了までに溜まる音声は送信開始時に破棄する
  micTaskActive = true;
}

// 録音タスク停止。I2S_NUM_0のuninstall前に必ず呼ぶ
void micCaptureStop() {
  if (!micRing) return;
  micTaskActive = false;
  delay(10);
  unsigned long start = millis();
  while (!micTaskParked && (millis() - start < 500)) delay(5);
}
...
void startSTTRecording() {
  ...
  micCaptureStart();  // 録音タスク起動（リングバッファ初期化）
  isRecording = true;
...
void setup() {
  Serial.begin(921600);
  delay(100);
  Serial.println("\n🚀 ToyTalker Mini v0.5 (experimental: mic jitter buffer)");
  ...
  // マイク送信ジッタバッファ初期化（PSRAM）+ 録音専用タスク起動
  micRing = (uint8_t*)ps_malloc(MIC_RING_SIZE);
  if (micRing) {
    xTaskCreatePinnedToCore(micCaptureTask, "mic", 8192, NULL, 3, &micTaskHandle, 0);
    Serial.printf("🎙️ Mic jitter buffer ready (%d KB)\n", MIC_RING_SIZE / 1024);
  } else {
    Serial.println("⚠️ micRing ps_malloc failed → direct send mode");
  }
...
void loop() {
  ...
  if (isRecording && wifiGotIP && ws.isConnected()) {
    ...
    if (micRing) {
      // 送信可能になった最初の瞬間、WS接続前に溜まった古い音声を破棄
      // （発話の頭を切らないよう直近300msだけ残す）。これをしないと
      // Sonioxが過去の無音を処理し終わるまで文字起こしが遅延する
      if (micFlushPending) {
        const size_t KEEP_BYTES = 9600;  // 16kHz×16bit×300ms
        if (micRingAvail() > KEEP_BYTES) {
          micRingTail = (micRingHead + MIC_RING_SIZE - KEEP_BYTES) % MIC_RING_SIZE;
        }
        micFlushPending = false;
      }
      // リングバッファ方式: 溜まっている分を1ループで全部送り切る（上限1秒分/回）。
      // 1チャンクずつだとloop()の周期に送信レートが律速されてbacklogが溜まる。
      // 回線が詰まってsendBINが失敗/ブロックしてもリングが吸収し、録音は欠落しない
      static uint8_t micTxBuf[MIC_SEND_CHUNK];  // 内部RAMステージング（仮説A対策:
      // WebSocketsClientはペイロードをin-placeマスクすることがあるため、リング本体を
      // 直接渡すと失敗→再送時に二重マスクの壊れたデータを送ってしまう）
      size_t budget = 32768;  // 暴走防止の上限（音声1秒分）
      while (budget > 0) {
        size_t avail = micRingAvail();
        if (avail == 0) break;
        size_t t = micRingTail;
        size_t n = min(avail, (size_t)MIC_SEND_CHUNK);
        n = min(n, MIC_RING_SIZE - t);  // 折返し前まで
        memcpy(micTxBuf, micRing + t, n);
        bool ok = ws.sendBIN(micTxBuf, n);
        if (!ok) {
          sendFail++;  // 失敗時はtailを進めず次のループで再送（元データは無傷）
          break;
        }
        micRingTail = (t + n) % MIC_RING_SIZE;
        sendOk++;
        budget -= n;
        ...
        ws.loop();  // 仮説B対策: 送信バースト中も受信(partial)処理を回す
      }
    } else if (millis() - lastSend > 5) {
      // フォールバック: 従来の直接読み取り→送信（micRing確保失敗時）
      int32_t raw[512];
      int16_t pcm[512];
      size_t n = 0;
    ...
    if (millis() - lastStats > 5000) {
      Serial.printf("[STT] send ok=%d fail=%d backlog=%uB RSSI=%d heap=%d\n", sendOk, sendFail, (unsigned)micRingAvail(), WiFi.RSSI(), ESP.getFreeHeap());
      sendOk = 0; sendFail = 0;
      lastStats = millis();
    }
  }
